...
def match_process_name_to_cmd(data):
    ...
    for process in data:
        if process != data[process]["CMD"]:
            print("Process name does not match process CMD. process name should be: " + data[process]["CMD"]
                  + " not " + process)
            process_error = True
    return (not process_error)
...
def manage_duplicates(pairs):
    ...
    for key, value in pairs:
        if key in diffs:
            raise ValueError("There are duplicate keys in the json file, the file is not valid")
        else:
            diffs.append(key)
    return (dict(pairs))
